...
# sort author affiliation dataframe by pmid variable (?):
...
# check--create a new dataframe (i.e., multi_type) that pulls the author_id2 of rows where the type_num value is more than 1:
...
```
## 4. Optional: Create Publication Attribute Data
...
#there is a value of 1 and grab the column's name for the new categorical variable, study_site_recode (?) 
...
#select useful columns to keep in the dataframe:
pubs <- pubs %>% 
  select(cycle, year_cite, month, day, date_publish, weeks_from_start, months_from_start, pmid)
...
#now created stacked data with authors on top
stacked.df <- bind_rows(author_affil_wide %>% select(all_of(shared_var_names)), 
                    pubs %>% select(all_of(shared_var_names)) )
...
attributes_authors_publications  <- bind_cols(shared, author_specific,
...
edges <- c(t(author_publication_edgelist %>% select(sender, receiver)))
